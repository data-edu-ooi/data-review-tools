#!/usr/bin/env python
"""
Created on Dec 11 2019 by Lori Garzio
@brief Automated analysis of HPIES data (had to download .nc files from THREDDS)
sDir: location to save summary output
ncdir: location of .nc files on local machine
"""
...
def main(sDir, ncdir, deployment_num):
    ...
    for r in rd_list:
        ...
        for rev_dep in review_deployments_int:
            ...
            datasets = [s for s in udatasets if rev_dep in s]
            datasets.sort()
            if len(datasets) > 0:
                notes = []
                time_ascending = ''
                ...
                for i in range(len(datasets)):
                    ds = xr.open_dataset(os.path.join(ncdir, datasets[i]), mask_and_scale=False)
                    ...
                    if i == 0:
                        fname, subsite, refdes, method, data_stream, deployment = cf.nc_attributes(os.path.join(ncdir, datasets[0]))
                ...
                if fname not in filenames:
                    data['deployments'][deployment]['method'][method]['stream'][data_stream]['file'][
                        fname] = OrderedDict(
                        file_downloaded=pd.to_datetime('20191206T184231').strftime('%Y-%m-%dT%H:%M:%S'),
...
if __name__ == '__main__':
    ...
    ncdir = '/Users/lgarzio/Documents/OOI/DataReviews/RS/RS01SLBS/RS01SLBS-LJ01A-05-HPIESA101/data'
    deployment_num = None  # None or int

    main(sDir, ncdir, deployment_num)
